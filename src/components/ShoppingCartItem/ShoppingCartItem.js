...
function ShoppingCartItem({ id, img, title, price, quantity, unitsInStock, handleChange, handleRemove }) {
	function onHandleChange(event) {
		handleChange(event, id);
	}

	function onHandleRemove(event) {
		handleRemove(id);
	}

	function buildSelectOptions(unitsInStock) {
		return Array.from({ length: unitsInStock }, (_value, key) => {
			return (
